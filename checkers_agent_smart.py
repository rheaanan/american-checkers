...
class problem_struct:
    def __init__(self, game_type="SINGLE", my_color="BLACK",time_left= 5 ):
...
def main():
    ...
    f = open("input_smart.txt", "r")
    #f2 = open("output.txt", "w")
    game_type = f.readline().rstrip()
    my_color = f.readline().rstrip()
    time_left = float(f.readline().rstrip())
    problem = problem_struct(game_type, my_color, time_left)
