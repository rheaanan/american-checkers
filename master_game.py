...
class problem_struct:
    ...
    def to_output_file(self, from_pos, moves_sequence, board_state):
        if len(moves_sequence)>1 or abs(from_pos[0] - moves_sequence[0][0])>1:              # jump moves chosen
            for to_pos in moves_sequence:
                print("J ", board_positions[from_pos]," ", board_positions[to_pos] )
                from_pos = to_pos
        else:                                                                               # normal move chosen
            to_pos = moves_sequence[0]
            print("E ", board_positions[from_pos]," ", board_positions[to_pos])
    ...
    def take_move(self, coin, jump, from_pos, to_pos, white_coin_pos, black_coin_pos):        # modifies board state according to move and returns
        new_white_state = copy.copy(white_coin_pos)
        new_black_state = copy.copy(black_coin_pos)
        if coin=='w' or coin=='W':
                ...
                if new_val in opponent_coins[coin]:  # opponents so we coin can jump
                    ...
                    if (valid):
                        ...
                        if jump =='.':
                            ...
                            coin_jump_series = copy.copy(jump_moves)
                            coin_jump_series.append((x, y))
                            new_white, new_black,new_coin = self.take_move(coin, True, pos, (x, y), white_coin_pos,
                                                                  black_coin_pos)
                            #print("coin",coin)
                            # if new_coin!=coin:
                ...
                if new_val in opponent_coins[coin]:                          # opponents coin so can jump
                    ...
                    if(valid):
                        ...
                        if jump == '.':
                            ...
                            coin_jump_series.append((x, y))
                            final_jump_list =[]
                            new_white, new_black, coin = self.take_move(coin, True, pos, (x, y), white_coin_pos, black_coin_pos)
                            self.jump_recursive((x,y),coin,new_white, new_black,coin_jump_series,final_jump_list)
                            if len(final_jump_list) == 0:
                                white_state, black_state, coin = self.take_move(coin, False, pos, (x, y), white_coin_pos, black_coin_pos)
                                final_jump_list.append(([(x, y)], (white_state, black_state)))
                            for moves_series in final_jump_list:
                                jump_moves.append(moves_series)
                elif new_val == '.' and len(jump_moves)==0:                             # empty space basic move check pnly if no jump moves found. or else dont waste time populating this
                    white_state,black_state, coin = self.take_move( coin, False, pos, (x,y), white_coin_pos, black_coin_pos)
    ...
    def generate_board_moves(self, color, state):
        ...
        if color == "WHITE":
            ...
            for key,value in white_coin_pos.items():
                #print(key)
                basic_moves, jumping_moves = self.gen_coins_moves(key, value, state)
    ...
    def alpha_beta_search(self, state):
        depth = 0
        v, from_pos, action, ret_state = self.max_value(state,(-8,-8),[(-7, -7)], neg_infinity, pos_infinity, depth)
        return from_pos, action, ret_state  ##! based on the value v chosen which action is to be taken

    def max_value(self, state, from_pos, action_taken, alpha, beta, depth):
        depth+=1
        action = action_taken
        ret_from_pos = from_pos
        print("depth", depth)
        if self.terminal_test(state,depth):
            return self.score_board(self.my_color, state), ret_from_pos, action, state
        v = neg_infinity
        my_actions, was_jump = self.generate_board_moves(self.my_color, state)
        print("my_action", my_actions)
        for a_key,a_value in my_actions.items():
            print(a_key, "value", a_value)
            for (move, state_temp) in a_value:
                temp_v = v
                v_m, from_pos_m, action_m, state_m = self.min_value(state_temp, a_key, move, alpha,beta,depth)
                v = max(v, v_m)
                if v!= temp_v:
                    action = move
    ...
    def min_value(self, state, from_pos, action_taken, alpha, beta, depth):
        depth+= 1
        action = action_taken
        ret_from_pos = from_pos
        print("depth",depth)
        if self.terminal_test(state, depth):
            return self.score_board(opponent_color[self.my_color], state), ret_from_pos, action, state
        v = pos_infinity
        opp_actions, was_jump = self.generate_board_moves(opponent_color[self.my_color], state)
        print("opp_action", opp_actions)
        for a_key, a_value in opp_actions.items():
            print(a_key, "value", a_value)
            for (move, state_temp) in a_value:
                temp_v = v
                v_m, from_pos_m, action_m, state_m =self.max_value(state_temp,a_key, move, alpha, beta, depth)
                v = min(v, v_m)
                if v != temp_v:
                    action = move
                    ret_state = state_temp
